    ...
    >
      <span :class="isDark ? 'text-gray-300' : 'text-neutral-400'">
        <svg width="18" height="18" fill="none" viewBox="0 0 24 24">
          <circle :stroke="iconColor" stroke-width="2" cx="11" cy="11" r="7" />
          <path :stroke="iconColor" stroke-width="2" d="M20 20l-3.5-3.5" />
        </svg>
      </span>
      <div class="relative flex-1">
        ...
        <button
          v-if="query"
          class="absolute right-1 top-1/2 -translate-y-1/2 p-1 rounded transition cursor-pointer"
...
const suggestions = computed(() => {
  ...
  return (fetchedMarkers.value || [])
    .filter((m: Marker) => (m.title || "").toLowerCase().includes(q))
    .slice(0, 8);
});
...
const move = (dir: number) => {
  ...
  activeIndex.value =
    (activeIndex.value + dir + suggestions.value.length) %
